    ...
    def extract_from_ocr(pdf, pages)
      ...
      if pages
        pages.each do |page|
          ...
          run "MAGICK_TMPDIR=#{tempdir} OMP_NUM_THREADS=8 gm convert +adjoin #{MEMORY_ARGS} #{OCR_FLAGS} #{pdf}[#{page - 1}] #{tiff} 2>&1"
        ...
        run "MAGICK_TMPDIR=#{tempdir} OMP_NUM_THREADS=8 gm convert #{MEMORY_ARGS} #{OCR_FLAGS} #{pdf} #{tiff} 2>&1"
